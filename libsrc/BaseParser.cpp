...
void BaseParser::setup(bool isNew)
{
	...
	if (!isNew)
	{
		...
		BaseParserPtr newPointer = shared_from_this();
...
void BaseParser::addChild(std::string category, ParserPtr child)
{
	if (!child) return;
...
ParserPtr BaseParser::getChild(std::string className, int num)
{
...
void BaseParser::addReference(std::string category, ParserPtr cousin)
{
	if (!cousin) return;
...
ParserPtr BaseParser::objectOfType(char *className)
{
...
BaseParserPtr BaseParser::processBlock(char *block)
{
	...
	*comma = '\0';
	...
	BaseParserPtr object = objectOfType(block);
	if (!object)
	{
...
void BaseParser::resolveReferences()
{
	     ...
	     it != _resolveList.end(); it++)
	{
		for (int j = 0; j < it->second.size(); j++)
		{
			...
			BaseParserPtr parser = resolveReference(path);
	     ...
	     it != _parserList.end(); it++) 
	{
		for (int j = 0; j < it->second.size(); j++)
		{
			BaseParserPtr child = it->second[j];
			if (!child) continue;
